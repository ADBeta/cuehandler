...
*******************************************************************************/
//TODO use static_cast<> instead of c casting
#include <string>
#include <list>
#include <fstream>
#include <iostream>
...
CueException track_push_overspec("Push To Track has an ID greater than 99");
CueException index_push_overspec("Push To Index has an ID greater than 99");
...
static bool IsValidTimestamp(const std::string &ts) {
	bool valid = false;
	if(ts.length() == 8 && ts[2] == ':' && ts[5] == ':') valid = true;
...
static std::string GetWordFromBack(const size_t word_index,
	...
	if(word_index > 0) {	
		//Current word_start(ws), word_end(we), and word_count(wc)
		//TODO change this to do while?
		size_t ws = std::string::npos - 1, we = std::string::npos - 1, wc = 0;	
...
//TODO Create exceptions for this to throw
int CueFile::Read(CueSheet &cs) {
	...
	while(getline(this->cue_file, line_str)) {
		...
		//Exit if the line is invalid TODO
		if(l_type == CueSheet::LineType::Invalid) return -3;
		...
		if(l_type == CueSheet::LineType::File) {
			...
			uint32_t fbytes = this->GetFileBytes( GetParentDir(this->filename)+fname);
			
			CueSheet::FileObj temp_file(fname, ftype, fbytes);
...
int CueFile::Write(const CueSheet &cs) {
	//Guard against use without a set filename, Attempt to open the file.
...
//TODO Revisit this and rethink it if possible
uint32_t CueFile::GetFileBytes(const std::string &fname) {	
	uint32_t bytes = 0;
	bool failflag = false;
	
	//Only continure if the filename string is not empty
	if((failflag = !fname.empty())) {
		std::fstream file(fname, std::ios::in | std::ios::binary);
		
		//Only continue if the file opened correctly
		if(!(failflag = !file.is_open())) {
			file.clear();
			file.seekg(0, file.end);
			bytes = (uint32_t)file.tellg();
			file.close();
		}
	}
	
	if(failflag) throw file_invalid;
	return bytes;
}
...
int CueSheet::PushTrack(FileObj::TrackObj *track_ptr) {
	if(track_ptr == nullptr) {
		throw track_push_null_input;
		return -1;
	}
	
	//TODO Add selectable guard for this
	if(track_ptr->id > 99) {
		throw track_push_overspec;
		return -2;
	}
	
	FileObj *f_last = this->GetLastFile();
	if(f_last == nullptr) {
		throw track_push_null_file;
		return -3;
	}
...
int CueSheet::PushIndex(FileObj::TrackObj::IndexObj *index_ptr) {
	if(index_ptr == nullptr) {
		throw index_push_null_input;
		return -1;
	}
	
	//TODO Add selectable guard for this
	if(index_ptr->id > 99) {
		throw index_push_overspec;
		return -2;
	}
	
	FileObj::TrackObj *t_last = this->GetLastTrack();
	if(t_last == nullptr) {
		throw index_push_null_track;
		return -3;
	}
...
//TODO SESR and exit before copying if error
int CueSheet::Combine(std::string op_filename) {
...
std::string CueSheet::BytesToTimestamp(const uint32_t bytes,
	...
	//TODO Throw?
	if(type == TrackType::Invalid || bytes == timestamp_nval) return "";
	
	std::string timestamp;
	uint16_t sect_bytes = GetSectorBytesInTrackType(type);
	
	//Make sure the bytes passed are perfectly divisible by sector bytes
	uint32_t sector_quo, sector_rem;
	DivideWithRemainder32(bytes, sect_bytes, sector_quo, sector_rem);
	//If the Bytes input does not neatly divide into the bytes-per-sector
	if(sector_rem != 0) return ""; //TODO Throw?
	...
	//If the Minutes value is greater than 99
	//TODO throw and add configurable guard to this
...
uint32_t CueSheet::TimestampToBytes(const std::string &timestamp, 
	...
	if(type != TrackType::Invalid || IsValidTimestamp(timestamp) != true) {
		//Strip values from the timestamp. "MM:SS:ff"
		uint16_t minutes = (uint16_t)std::stoul(timestamp.substr(0, 2));
		uint16_t seconds = (uint16_t)std::stoul(timestamp.substr(3, 2));
		uint16_t frames  = (uint16_t)std::stoul(timestamp.substr(6, 2));
		
		//75 sectors per second, convert mins to secs, plus the leftover 'frames'
		//Which are worth 1 sector each
		uint32_t sectors = (uint32_t)((seconds + (minutes * 60)) * 75) + frames;
		uint16_t sect_bytes = GetSectorBytesInTrackType(type);
		total_bytes = sectors * sect_bytes;
		
		//Make sure the bytes value is divisible by the sector bytes.
		if(total_bytes % sect_bytes != 0) total_bytes = timestamp_nval;
	}
